...
import { LogOut, LayoutDashboard, User, Store, X, Mail, Lock, Sparkles, ChefHat, ChevronDown, UserCircle, LogIn, UserPlus } from 'lucide-react';
...
const UserLayout = () => {
    ...
    return (
        <div className="min-h-screen bg-gradient-to-br from-orange-50 to-white flex flex-col font-sans">
            {/* Premium Global Navbar */}
            <nav className="bg-white/80 backdrop-blur-2xl border-b border-orange-50 sticky top-0 z-50 h-[100px] flex items-center shadow-sm">
                <div className="max-w-7xl mx-auto px-8 w-full flex justify-between items-center">
                    ...
                    <Link to="/" className="flex items-center space-x-3 group">
                        ...
                        <span className="text-2xl font-black text-slate-900 tracking-tighter uppercase relative">
                            DineSpot
                    ...
                    <div className="flex items-center space-x-2">
                            ...
                            <AnimatePresence>
                                {showUserDropdown && (
                                    <motion.div
                                        ...
                                        className="absolute right-0 mt-4 w-64 bg-white rounded-[2rem] shadow-2xl border border-orange-100/50 p-4 ring-1 ring-orange-50 overflow-hidden"
                                    >
                                        <div className="p-4 border-b border-orange-50 mb-2">
                                            <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Access Panel</p>
                                        </div>
                                        {user ? (
                                            <div className="space-y-1">
                                                <Link
                                                    ...
                                                    className="flex items-center space-x-3 px-5 py-3 rounded-xl hover:bg-orange-50 text-slate-700 hover:text-orange-600 transition-all font-bold text-xs"
                                                >
                                                ...
                                                <button
                                                    ...
                                                    className="w-full flex items-center space-x-3 px-5 py-3 rounded-xl hover:bg-rose-50 text-slate-700 hover:text-rose-600 transition-all font-bold text-xs"
                                                >
                                            ...
                                            <div className="space-y-1">
                                                <Link
                                                    to="/login?role=user"
                                                    onClick={() => setShowUserDropdown(false)}
                                                    className="flex items-center space-x-3 px-5 py-3 rounded-xl hover:bg-orange-600 hover:text-white text-slate-700 transition-all font-bold text-xs group"
                                                >
                                                ...
                                                <Link
                                                    to="/signup?role=user"
                                                    onClick={() => setShowUserDropdown(false)}
                                                    className="flex items-center space-x-3 px-5 py-3 rounded-xl hover:bg-slate-900 hover:text-white text-slate-700 transition-all font-bold text-xs group"
                                                >
                        ...
                        <Link
                            to="/staff/login"
                            className="flex items-center space-x-3 px-6 py-3 rounded-2xl font-black text-sm uppercase tracking-widest text-slate-600 hover:bg-slate-900 hover:text-white transition-all border border-transparent hover:shadow-xl hover:shadow-slate-200"
                        >
